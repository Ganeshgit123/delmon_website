...
{  
  ...
  {   
    ...
    $Return = array('result'=>'', 'error'=>'');
      ...
      if(is_uploaded_file($_FILES['file_ar']['tmp_name'])) {
          $allowed =  array('pdf','PDF');
          $filename = $_FILES['file_ar']['name'];
          $ext = pathinfo($filename, PATHINFO_EXTENSION);
          if(in_array($ext,$allowed)){
            $tmp_name = $_FILES["file_ar"]["tmp_name"];
            $profile = "uploads/images/about/";
            $name = basename($_FILES["file_ar"]["name"]);
            $newfilename = $name ;
            move_uploaded_file($tmp_name, $profile.$newfilename);
            $pdf_ar  = $newfilename;
      }else {
  ...
  {   
      ...
      if(empty($session)){ 
       ...
       $pdf_ar =  $this->input->post('old_file_ar');
 ...
 public function add_gallery_type()
      ...
      $attachment  = $_FILES["image"]['name'];
          ...
          for($i = 0; $i < sizeof($attachment); $i++)
          {
              $n++;
                if(is_uploaded_file($_FILES['image']['tmp_name'][$i]))
                {
                  ...
                  if(in_array($ext,$allowed))
                  {
                      ...
                      $profile = "uploads/images/gallery/";
                      $set_img = base_url()."uploads/images/gallery/";
                      $name = basename($_FILES["image"]["name"][$i]);
                      $newfilename = 'img_'.round(microtime(true)).$n.'.'.$ext;
                      move_uploaded_file($tmp_name, $profile.$newfilename);
                      $fname = $newfilename;
                    $images[] =  $fname ;

                    $uploadData[$i]['type'] = $inserid; 
                    $uploadData[$i]['image'] = $fname; 
                  }else
                  {
                      $Return['error'] = "Invalid file format";
                      $this->output($Return);
            ...
            if(!empty($uploadData)){ 
              ...
              $insert = $this->db->insert_batch('gallery', $uploadData);            
            } 

      if ($result == TRUE) {
        $Return['result'] = 'Gallery Added successfully.';
      } else {
        $Return['error'] =  'Bug. Something went wrong, please try again';
      }
      $this->output($Return);
  ...
  public function edit_gallery_type()
  {
        $session = $this->session->userdata('userdet');
         ...
         $id = $this->uri->segment(3) ;
		 $data['imgdata'] = $this->Admin_model->get_all_data('gallery',array('type'=>$id)) ;
  ...
  {   
      ...
      if(empty($session)){ 
       ...
       if($this->input->post('name')==='') {
        ...
        $inserid =  $this->db->insert_id();
        if ($result == TRUE) {
           $Return['result'] = 'Gallery updated successfully.';
  ...
  public function deleteImage(){ 
    ...
    if($this->input->post('id')){ 
        $id = $this->input->post('id'); 
        $imgData = $this->getImgRow($id); 
         
        // Delete image data 
        $con = array('id' => $id); 
        $delete = $this->db->delete('gallery', $con); 
         
        if($delete){ 
            // Remove files from the server  
            @unlink('uploads/images/gallery/'.$imgData['image']);  
            $status = 'ok';  
        } 
    } 
    echo $status;die;  
  ...
  public function commiteetype()
  {
    $session = $this->session->userdata('userdet');
    if(empty($session)){ 
      redirect('admin');
    }
    $data['data'] = $this->Admin_model->get_all_data('committee_type'); 
    $this->load->view('admin/committee_type', $data); 
  }

  public function new_commiteetype()
  {
    $session = $this->session->userdata('userdet');
    if(empty($session)){ 
      redirect('admin');
    }
    $this->load->view('admin/new_committee_type');
  }

  public function add_commiteetype()
  {   
     $session = $this->session->userdata('userdet');
    if(empty($session)){ 
      redirect('admin');
    ...
    $Return = array('result'=>'', 'error'=>'');
      ...
      $result = $this->db->insert('committee_type',$data);
      $inserid =  $this->db->insert_id();
  ...
  public function edit_commiteetype()
  {
      $session = $this->session->userdata('userdet');
      if(empty($session)){ 
         redirect('admin');
      ...
      $id = $this->uri->segment(3) ;
      $data['data'] = $this->Admin_model->get_single_data('committee_type',array('id'=>$id)) ;
      $this->load->view('admin/edit_committeetype', $data);
  }

  public function update_commiteetype()
  {   
      $session = $this->session->userdata('userdet');
      if(empty($session)){ 
          redirect('admin');
       ...
       if($this->input->post('name')==='') {
        ...
        $result = $this->Admin_model->update_data('committee_type',array('id'=>$id),$data);
        $inserid =  $this->db->insert_id();
        if ($result == TRUE) {
  ...
  public function managetype()
  {
    ...
    $data['data'] = $this->Admin_model->get_all_data('commitee_master_type'); 
    $this->load->view('admin/manage_type', $data); 
  }

  public function edit_managetype()
  {
      $session = $this->session->userdata('userdet');
      if(empty($session)){ 
         redirect('admin');
      ...
      $id = $this->uri->segment(3) ;
      $data['data'] = $this->Admin_model->get_single_data('commitee_master_type',array('id'=>$id)) ;
      $this->load->view('admin/edit_managetype', $data);
  }

  public function update_managetype()
  {   
      $session = $this->session->userdata('userdet');
      if(empty($session)){ 
          redirect('admin');
       ...
       if($this->input->post('name')==='') {
        ...
        $result = $this->Admin_model->update_data('commitee_master_type',array('id'=>$id),$data);
        $inserid =  $this->db->insert_id();
        if ($result == TRUE) {
           $Return['result'] = 'Data updated successfully.';         
        } else {
   ...
   public function manage_delete_entry()
  {  
   $session = $this->session->userdata('userdet');
    if(empty($session)){ 
      redirect('admin');
    ...
    $id = $this->input->post('id');
    $table = $this->input->post('table');
    if($table == "committee_type")
    {
      $this->Admin_model->delete_data("committee_type",array('id'=>$id)); 
      $this->Admin_model->delete_data("management",array('sub_type'=>$id)); 

    }else if($table == "management")
    {
      $this->Admin_model->delete_data("management",array('id'=>$id)); 
    } 
    return $id ; 
  }
